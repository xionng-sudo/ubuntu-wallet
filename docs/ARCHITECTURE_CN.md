...
## 主要职责
- 获取多币种行情数据（Phase 1: BTCUSDT/ETHUSDT/SOLUSDT/BNBUSDT；Phase 2: XRPUSDT/DOGEUSDT/ADAUSDT）
- 将每个交易对的 K 线写入独立子目录：`data/<SYMBOL>/klines_{1h,4h,1d,15m}.json`
- 配置方式：`SYMBOLS=BTCUSDT,ETHUSDT,...` 或 `ENABLE_PHASE2_SYMBOLS=true`
- 主交易对配置：`PRIMARY_SYMBOL`（默认 ETHUSDT，必须包含在 SYMBOLS 中）。主交易对额外采集 1m/5m，并用于特征计算、/api/market、健康检查。
- 向下兼容：`LEGACY_ETHUSDT_COMPAT=true`（默认）同时将**主交易对**（默认 ETHUSDT）数据写入根目录 `data/klines_*.json`，保持旧消费者语义不变。迁移完成后设置为 `false`。
