...
// ---- Klines (Binance Spot api/v3/klines) ----
...
func parseBinanceKlines(body []byte, symbol, interval string) ([]models.OHLCV, error) {
	var raw [][]interface{}
...
// GetKlines fetches candlestick data (latest window only).
// NOTE: Binance usually caps limit at 1000. For longer history use GetKlinesLookback().
func (b *BinanceCollector) GetKlines(symbol, interval string, limit int) ([]models.OHLCV, error) {
	...
	kl, err := parseBinanceKlines(body, symbol, interval)
	if err != nil {
		return nil, err
	}

	log.Infof("[Binance] Fetched %d klines for %s interval=%s", len(kl), symbol, interval)
	return kl, nil
}

// GetKlinesRange fetches klines within [startTime, endTime] (inclusive-ish), limited to 'limit' rows.
func (b *BinanceCollector) GetKlinesRange(symbol, interval string, limit int, startTime, endTime *time.Time) ([]models.OHLCV, error) {
	q := url.Values{}
	q.Set("symbol", symbol)
	...
	u := "https://api.binance.com/api/v3/klines?" + q.Encode()
	...
	return parseBinanceKlines(body, symbol, interval)
}

func uniqSortKlines(in []models.OHLCV) []models.OHLCV {
	seen := make(map[int64]models.OHLCV, len(in))
	for _, k := range in {
		seen[k.Timestamp.UTC().UnixMilli()] = k
	}

	out := make([]models.OHLCV, 0, len(seen))
	for _, v := range seen {
		out = append(out, v)
	}
...
// GetKlinesLookback fetches up to lookbackDays of klines by paging backwards.
// This is used to build meaningful 15m history for backtests.
func (b *BinanceCollector) GetKlinesLookback(symbol, interval string, lookbackDays int) ([]models.OHLCV, error) {
	if lookbackDays <= 0 {
		return b.GetKlines(symbol, interval, 500)
	}

	stepDur, err := intervalToDuration(interval)
	if err != nil {
		return nil, err
	}

	// Binance cap is typically 1000.
	const pageLimit = 1000

	now := time.Now().UTC()
	start := now.Add(-time.Duration(lookbackDays) * 24 * time.Hour)

	// Page forward from start -> now using startTime
	all := make([]models.OHLCV, 0, lookbackDays*24*4)
	cursor := start

	// Safety: maximum pages (avoid infinite loop if API returns same timestamp)
	maxPages := 20000

	for pages := 0; pages < maxPages; pages++ {
		// endTime not set; Binance returns next 'limit' bars starting from startTime
		kl, err := b.GetKlinesRange(symbol, interval, pageLimit, &cursor, nil)
		if err != nil {
			return nil, err
		}
		if len(kl) == 0 {
		...
		last := kl[len(kl)-1].Timestamp.UTC()

		// Advance cursor to just after last bar open time.
		next := last.Add(stepDur)
		if !next.After(cursor) {
			// protect against stuck cursor
			next = cursor.Add(stepDur)
		}
		cursor = next

		// Stop if we reached "now" (or last bar is beyond now)
	...
	out := uniqSortKlines(all)

	// Trim just in case (keep only >= start)
	trim := make([]models.OHLCV, 0, len(out))
	for _, k := range out {
		if !k.Timestamp.Before(start) && !k.Timestamp.After(now.Add(2*stepDur)) {
			trim = append(trim, k)
		}
	}

	log.Infof("[Binance] Lookback klines: symbol=%s interval=%s days=%d rows=%d", symbol, interval, lookbackDays, len(trim))
	return trim, nil
}
