...
| Phase 2 | XRPUSDT, DOGEUSDT, ADAUSDT | ✅ 已启用 |
...
```bash
# 全部 7 个交易对（默认，无需额外配置；configs/symbols.yaml 中均已启用）
...
> 全部 7 个交易对均已启用（`enabled: true`）。如需临时停用某币种，将 `configs/symbols.yaml` 中对应条目改为 `enabled: false`。
...
要启用或禁用某币种，修改对应的 `enabled: true/false` 即可。

### 10.3 按币种训练

```bash
# 单币种（自动从 configs/symbols.yaml 读取参数）
...
# 预演
bash scripts/train_all_symbols.sh --dry-run
...
每 60s:
  ...
  go-collector → POST /predict (symbol=XRPUSDT)  → data/XRPUSDT/predictions_log.jsonl
  go-collector → POST /predict (symbol=DOGEUSDT) → data/DOGEUSDT/predictions_log.jsonl
  go-collector → POST /predict (symbol=ADAUSDT)  → data/ADAUSDT/predictions_log.jsonl
```
